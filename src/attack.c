...
bool isSquareAttacked(int square, int side, BOARD *pos)
{
    ...
    // pawns
    if (side == WHITE && (pos->pieces[square + 11] == wP || pos->pieces[square + 9] == wP)) return true;
    if (side == BLACK && (pos->pieces[square - 11] == bP || pos->pieces[square - 9] == bP)) return true;

    // knights
    for (int i = 0; i < 8; i++) {
        int piece = pos->pieces[square + KnightDirection[i]];
        if (piece != OFFBOARD && isPieceKnight[piece] && pieceColor[piece] == side) return true;
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        while (piece != OFFBOARD) {
            ...
            if (isPieceRookOrQueen[piece] && pieceColor[piece] == side) return true;
            break;
        }
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        while (piece != OFFBOARD) {
            ...
            if (isPieceBishopOrQueen[piece] && pieceColor[piece] == side) return true;
            break;
        }
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (piece != OFFBOARD && isPieceKing[piece] && pieceColor[piece] == side) return true;
    }
